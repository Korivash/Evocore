...
module.exports = {
  async postRoles(client) {
    ...
    const embed = new EmbedBuilder()
      .setTitle("Cryptic Net Self Roles")
      .setDescription("Click a button to assign or remove a **Class** or **Role** instantly!")
      .addFields(
        { name: "Classes", value: "Choose your WoW classes below.", inline: true },
        { name: "Roles", value: "Choose your preferred role in groups.", inline: true }
      )
    ...
    const buildRows = keys => {
      ...
      keys.forEach((key, i) => {
        ...
        row.addComponents(new ButtonBuilder().setCustomId(key).setLabel(label).setStyle(style));
        if ((i + 1) % 5 === 0) {
          rows.push(row);
    ...
    await channel.send({ embeds: [embed], components: [...classRows, ...roleRows] });
  }
};
